...
const ProductDetails = () => {
      ...
      reviewsList: [
        {
          ...
          date: '2024-11-01',
          comment:
            'Excellent quality and very comfortable. The fit is perfect and the fabric feels premium.'
        },
        {
          user: 'Mariam A.',
          rating: 5,
          date: '2024-10-12',
          comment:
            'Stylish and minimalist, exactly what I was looking for. Highly recommended!'
        },
        {
          user: 'Omar K.',
          rating: 4,
          date: '2024-09-28',
          comment:
            'Great piece and great value. I suggest sizing up for a more relaxed fit.'
        }
      ],
    },
    '2': {
      ...
      reviewsList: [
        {
          ...
          rating: 5,
          date: '2024-08-14',
          comment: 'Very soft and breathable fabric. The olive color looks amazing.'
        },
        {
          user: 'Nour H.',
          rating: 4,
          date: '2024-08-09',
          comment: 'Comfortable everyday tee with a premium feel.'
        }
      ],
    },
    '3': {
      ...
      reviewsList: [
        {
          ...
          rating: 5,
          date: '2024-12-02',
          comment: 'Beautifully crafted jacket. Feels luxurious and looks sharp.'
        },
        {
          user: 'Hassan R.',
          rating: 5,
          date: '2024-11-17',
          comment: 'Perfect for winter nights. The details are impressive.'
        },
        {
          user: 'Laila F.',
          rating: 4,
          date: '2024-11-03',
          comment: 'Love the two-tone design. Warm and comfortable.'
        }
      ],
    }, 
  } as const;
  ...
  const hasNumericPrice = (product as any).priceValue !== undefined;
  const currencyCode = (product as any).currency || 'EGP';
  const discountPercent = (product as any).discountPercent || 0;
  const priceValue = (product as any).priceValue as number | undefined;
  const discountedValue =
    hasNumericPrice && priceValue !== undefined
      ? priceValue * (1 - discountPercent / 100)
      : undefined;

  const formatCurrency = (value: number) => {
    try {
      const localeForCurrency = isRTL ? 'ar-EG' : 'en-EG';
    ...
    <>
      ...
      <section className="py-8 bg-muted/30">
        <div className="container mx-auto px-4">
          <div
            className={`flex items-center space-x-2 text-sm font-inter ${isRTL ? 'rtl:space-x-reverse' : 'ltr:space-x-reverse'
              }`}
          >
            <Link
              href="/"
          ...
          <div className="grid lg:grid-cols-2 gap-12">
            ...
            <div className={isRTL ? 'lg:order-2' : ''}>
              <div className="sticky top-24">
                ...
                {/* Image Thumbnails */}
                <div
                  className={`flex space-x-4 ${isRTL ? 'rtl:space-x-reverse' : 'ltr:space-x-reverse'
                    }`}
                >
                  {product.images.map((image, index) => (
                    <button
                      ...
                      className={`aspect-square w-20 rounded-lg overflow-hidden border-2 transition-quick ${selectedImage === index
                        ? 'border-secondary'
                        : 'border-transparent'
                        }`}
                    >
            ...
            <div className={isRTL ? 'lg:order-1' : ''}>
              ...
              <div className="mb-8">
                ...
                <div
                  className={`flex items-center space-x-4 mb-6 ${isRTL ? 'rtl:space-x-reverse' : 'ltr:space-x-reverse'
                    }`}
                >
                  <div
                    className={`flex items-center space-x-1 ${isRTL ? 'rtl:space-x-reverse' : 'ltr:space-x-reverse'
                      }`}
                  >
                    {[...Array(5)].map((_, i) => (
                      <Star
                  ...
                  <div
                    className={`flex items-center space-x-2 ${isRTL ? 'rtl:space-x-reverse' : 'ltr:space-x-reverse'
                      }`}
                  >
                    <Button
                      variant="ghost"
                ...
                <div className="mb-6 flex items-center gap-4">
                  {hasNumericPrice && discountedValue !== undefined ? (
                    <>
                      <span className="font-cinzel text-3xl font-bold text-secondary">
                        {formatCurrency(discountedValue)}
                      </span>
                      {discountPercent > 0 && priceValue !== undefined && (
                        <>
                          <span className="text-muted-foreground line-through font-cinzel text-xl">
                    ...
                    <p className="font-cinzel text-2xl font-semibold text-secondary">
                      {currentPrice}
                    </p>
                  )}
                </div>
                ...
                <div className="grid grid-cols-6 gap-3">
                  {product.sizes.map((size) => (
                    <button
                      ...
                      className={`aspect-square border-2 rounded-lg font-inter font-medium transition-quick ${selectedSize === size
      ...
      <section className="py-12" ref={reviewsRef} id="reviews">
        <div className="container mx-auto px-4">
          ...
          <div className="space-y-4">
            {((product as any).reviewsList || []).map(
              (review: { user: string; rating: number; date: string; comment: string }, index: number) => (
                <Card key={index} className="shadow-soft border-0">
                  <CardContent className="p-6">
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        <p className="font-inter font-semibold text-primary">{review.user}</p>
                    ...
                    <p className="text-muted-foreground font-inter leading-relaxed">{review.comment}</p>
                  </CardContent>
                </Card>
              )
            )}
